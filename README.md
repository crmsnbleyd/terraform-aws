...
- This project uses a unified pipeline in [`.github/workflows/main.yml`](https://github.com/crmsnbleyd/terraform-aws/blob/main/.github/workflows/main.yml).
- Triggering: The workflow runs on every push to main, every PR, or can be manually triggered via workflow_dispatch.
...
- OIDC Provider: Transition from long-lived AWS IAM User Secrets to GitHub Actions OIDC for short-lived, identity-based credentials.
- Remote State: Implement an S3 backend with DynamoDB locking for team-based state management.
- Real ECR Integration: Replace the mock push with a real ECR repository and image scanning (Amazon Inspector) for vulnerability detection.
- Auto-scaling: Add App Auto Scaling policies based on CPU/Memory utilization for the Fargate service.
